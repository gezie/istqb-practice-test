...
import { BookOpenCheck, ClipboardCheck, LoaderCircle, PlayCircle, Zap } from "lucide-react"
...
export function TestConfiguration() {
  ...
  return <section className="mx-auto mt-10 max-w-4xl rounded-2xl border bg-card p-5 shadow-sm sm:p-7">
    ...
    <div role="radiogroup" aria-label="Režim testu" className="mt-5 grid gap-3 md:grid-cols-3">
      {MODES.map(({ id, title, subtitle, description, icon: Icon }) => <button type="button" role="radio" aria-checked={mode === id} key={id} onClick={() => setMode(id)} className={cn("min-h-44 rounded-xl border-2 p-4 text-left transition", mode === id ? "border-primary bg-accent" : "hover:border-primary/40")}>
    ...
    {mode === "chapters" && <fieldset className="mt-5 rounded-xl bg-muted/60 p-4"><legend className="px-1 text-sm font-semibold">Vyberte jednu nebo více kapitol</legend><div className="mt-2 grid gap-3 sm:grid-cols-2">{CHAPTERS.map((name, index) => <label key={name} className="flex min-h-11 cursor-pointer items-center gap-3 text-sm"><input type="checkbox" className="size-5 accent-primary" checked={selected.includes(index + 1)} onChange={() => setSelected(value => value.includes(index + 1) ? value.filter(item => item !== index + 1) : [...value, index + 1])}/><span>{name}</span></label>)}</div></fieldset>}
    <Button size="lg" className="mt-6 w-full" disabled={loading || (mode === "chapters" && !selected.length)} onClick={start}>{loading?<LoaderCircle className="animate-spin"/>:<PlayCircle/>}{loading?"Načítání…":<>Spustit {mode === "exam" ? "ostrý test" : "procvičování"}</>}</Button>
  </section>
}
